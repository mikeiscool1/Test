...
const scoreQuery = fs_1.default.readFileSync('./sql/score.sql').toString();
...
app.get('/api/:testId', async (req, res) => {
    ...
    if (!authorization)
        return void res.status(401).send({ message: 'Missing ticket.' });
    const test = await db.test.findUnique({ where: { id: testId } });
    if (!test)
        return void res.status(404).send({ message: 'Test not found.' });
    if (!test.tickets.includes(authorization))
        return void res.status(403).send({ message: 'Invalid ticket.' });
    delete test.tickets;
...
app.post('/api/:testId/begin', async (req, res) => {
    ...
    if (!module)
        return void res.status(500).send({ message: 'Could not locate the module.' });
    ...
    if (!questions)
        return void res.status(500).send({ message: 'Could not locate the questions.' });
...
app.post('/api/:testId/answers', async (req, res) => {
    ...
    if (!answer)
        return void res.sendStatus(404);
    if (answer.invalid)
        return void res.status(403).send({ message: 'Your test was invalidated.', code: 0 });
    if (answer.finished)
        return void res.status(403).send({ message: 'You already finished the test.', code: 1 });
    if (answer.module_answers_received)
        return void res.status(403).send({ message: 'You already submitted the answers for this module. Go to the next module.', code: 2 });
    const module = await db.module.findUnique({
    ...
    if (answers.length !== questions.length)
        return void res.status(400).send({ message: 'Mismatch number of questions.' });
    ...
    for (let i = 0; i < answers.length; i++) {
        ...
        if (typeof ans !== 'number')
            return void res.status(400).send({ message: `Question ${i + 1}: invalid answer format.` });
        if (ans >= questions[i].choices.length)
            return void res.status(400).send({ message: `Question ${i + 1}: invalid answer.` });
    }
    await db.answer.update({
        where: { ticket_test_id: { ticket: authorization, test_id: testId } },
        data: {
...
app.get('/api/:testId/next', async (req, res) => {
    ...
    if (answer.finished)
        return void res.status(403).send({ message: 'You already finished the test.', code: 1 });
    if (!answer.module_answers_received)
        return void res.status(403).send({ message: 'Submit answers from the previous module before continuing. Even if the time is up, still request to submit your answers, and continue after getting an error.', code: 2 });
    const module = await db.module.findUnique({
        where: { number_test_id: { number: answer.current_module + 1, test_id: testId } },
        include: { test: true }
    });
...
const convertPrismaResult = (result) => {
    return {
...
app.get('/api/:testId/results', async (req, res) => {
    ...
    if (!answer)
        return void res.status(404).send({ message: 'Submission not found.' });
    if (answer.invalid)
        return void res.status(403).send({ message: 'Your test was invalidated.', code: 0 });
    if (!answer.finished)
        return void res.status(403).send({ message: 'You have not finished the test.', code: 1 });
    if (!answer.test.results_available)
        return void res.status(403).send({ message: 'Results are not available.', code: 2 });
    const scoreArr = await db.$queryRawUnsafe(scoreQuery, authorization);
    if (scoreArr.length === 0)
        return void res.sendStatus(500);
    const score = convertPrismaResult(scoreArr[0]);
    const { test } = answer;
    switch (test.review_permission) {
        case client_1.ReviewPermission.NONE:
            delete score.points_earned;
            delete score.total_possible_points;
            delete score.percentage;
            delete score.total_questions;
            delete score.total_wrong;
            delete score.wrong_answer_indices;
            return void res.send({ score, review_permission: client_1.ReviewPermission.NONE });
        case client_1.ReviewPermission.NUMBER_WRONG:
            delete score.wrong_answer_indices;
            return void res.send({ score, review_permission: client_1.ReviewPermission.NUMBER_WRONG });
        case client_1.ReviewPermission.QUESTION_NUMBERS:
            test.modules.forEach(m => m.questions.forEach((q) => {
                delete q.answer;
                delete q.explanation;
            }));
            return void res.send({ score, modules: test.modules, review_permission: client_1.ReviewPermission.QUESTION_NUMBERS });
        case client_1.ReviewPermission.ANSWERS:
            return void res.send({ score, modules: test.modules, your_answers: answer.answers, review_permission: client_1.ReviewPermission.ANSWERS, review_point_weight: test.review_point_weight });
    }
});
