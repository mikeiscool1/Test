...
export function main() {
  ...
  const id = document.getElementById('test-id') as HTMLInputElement;
  const ticketElement = document.getElementById('ticket') as HTMLInputElement;
  ...
  async function checkCurrent() {
    ...
    if (!req.ok) {
      if (req.status === 403) {
        ...
        if (res.code === 1 || res.code === 0) {
          if (id.value) localStorage.setItem('test_id', id.value);
    ...
    if (res.refresh_start_time) localStorage.setItem('start_module_time', Date.now().toString())
  ...
  enterBtn.addEventListener('click', async e => {
    if (!id.value) return errMsg.innerHTML = 'Test ID is a required field.';
    if (!ticketElement.value) return errMsg.innerHTML = 'Ticket is a required field.';
