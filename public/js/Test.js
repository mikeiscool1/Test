...
let eReferenceImg = document.getElementById('reference-img');
...
export class Test {
    ...
    constructor() {
        ...
        eReferenceImg = document.getElementById('reference-img');
    ...
    renderEverything() {
        ...
        if (!this.module.reference_image)
            eReference.classList.add('d-none');
        else {
    ...
    renderQuestion() {
        ...
        if (this._currentQuestion == 0)
            eBack.classList.add('disabled');
        else
            eBack.classList.remove('disabled');
    ...
    nextQuestion() {
        if (this._currentQuestion == this.module.questions.length - 1)
            throw new Error('nextQuestion(): end of module.');
        this.currentQuestion = this._currentQuestion + 1;
    }
    backQuestion() {
        if (this._currentQuestion == 0)
            throw new Error('backQuestion(): already at question 0.');
        this.currentQuestion = this._currentQuestion - 1;
    }
    async begin(testId, ticket) {
    ...
    async submitAnswers() {
        ...
        if (req.status === 404)
            return alert('Failed to submit answers: this test no longer exists.');
        const res = await req.json();
        if (!req.ok) {
    ...
    async requestNextModule() {
        ...
        if (!req.ok) {
            if (req.status === 500)
                return alert('An error occoured.');
            const res = await req.json();
            return alert(res.message);
        }
        const res = await req.json();
