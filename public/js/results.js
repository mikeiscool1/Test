...
export async function main() {
    ...
    const { score, modules, your_answers, review_permission, review_point_weight } = res;
    eScore.innerHTML = score.score;
    if (review_permission === ReviewPermission.NONE)
        return;
    ...
    if (review_permission === ReviewPermission.NUMBER_WRONG)
        return;
        ...
        if (target.classList.contains('btn-popup-close')) {
            ...
            for (const c of question.choices) {
                ...
                if (question.answer === i)
                    li.classList.add('bg-correct');
                else if (+yourAnswer === i)
                    li.classList.add('bg-incorrect');
